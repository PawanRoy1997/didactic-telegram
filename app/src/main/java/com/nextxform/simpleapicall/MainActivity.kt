...
class MainActivity : ComponentActivity() {
    ...
    override fun onStart() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            HttpRequest("https://pokeapi.co/api/v2/pokemon/ditto", HashMap(), object :HTTPCallback{
                override fun processFinished(output: String) {
                    Log.d("RESPONSE", output)
                }

                override fun processFailed(responseCode: Int, output: String) {
                    Log.d("RESPONSE", "processFailed: $responseCode $output")
                }
            }).performCall()
        }
    }
